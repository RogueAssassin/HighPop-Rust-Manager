...
public class WebApiService : IDisposable
{
    ...
    public void Start(int port, string token)
    {
        if (port is <= 0 or > 65535)
            throw new ArgumentOutOfRangeException(nameof(port), "Web API port must be between 1 and 65535.");
        if (string.IsNullOrWhiteSpace(token))
            throw new InvalidOperationException("Web API cannot start without an access token.");
        Stop();
        Port  = port;
        Token = token;
    ...
    private static void AddFirewallRule(int port)
    {
        try
        {
            var name = $"HighPop Web API port {port}";
    ...
    private async Task ListenLoop(CancellationToken ct)
    {
        while (!ct.IsCancellationRequested)
        {
            ...
            _ = Task.Run(() => HandleRequest(ctx), ct)
                    .ContinueWith(t => Console.WriteLine($"[HighPop] API request error: {t.Exception?.InnerException?.Message}"),
    ...
    private async Task HandleRequest(HttpListenerContext ctx)
    {
        ...
        if (path == "" || path == "/" || path == "/ui")
        {
            ...
            {
                resp.StatusCode = 403;
                await SendHtml(resp, "<html><body style='font-family:sans-serif;background:#0d1117;color:#8b949e;display:flex;align-items:center;justify-content:center;height:100vh;margin:0'><p>Web dashboard is disabled. Enable it in HighPop Settings → Web Remote Control.</p></body></html>");
                return;
            }
        ...
        {
            var raw   = req.Headers["Authorization"] ?? string.Empty;
            var token = raw.StartsWith("Bearer ", StringComparison.OrdinalIgnoreCase) ? raw[7..] : raw;

            bool isApiToken  = token.Length > 0
    ...
    private static byte[]? ReadLogoBytes()
    {
        ...
        // window/taskbar icon, not the "HighPop" text logo, since the shield reads more clearly at
        // the small sizes browser tabs and inline headers render it at.
        try
        {
            var uri    = new Uri("pack://application:,,,/HighPop;component/assets/brand/highpop.ico", UriKind.Absolute);
...
.wake-btn{width:100%;margin-top:16px;background:#f05a28;border:none;border-radius:6px;padding:10px;color:#fff;font-size:14px;font-weight:600;cursor:pointer}
...
<body>
<div class="card">
  <h1><img src="/favicon.ico" alt="HighPop" style="height:24px;vertical-align:middle;margin-right:8px;border-radius:4px"/>{{Esc(srv.DisplayName)}}</h1>
  ...
  <ul>{{playerRows}}</ul>
  {{wakeButtonHtml}}
  <div class="footer">Powered by HighPop Rust Manager</div>
</div>
</body>
</html>
""";
...
.field input:focus{border-color:#f05a28}
...
.srv:hover{border-color:#f05a2844}
/* Server card top */
...
.dot.Starting,.dot.Stopping,.dot.Installing,.dot.Updating{color:#f05a28;background:#f05a28}
...
.bb{background:#f05a28;color:#fff}
...
.con-row input:focus{border-color:#f05a28}
/* Log panel */
...
.notes-area textarea:focus{border-color:#f05a28}
...
<body>
<div class="hdr">
  <div class="hdr-title"><img src="/favicon.ico" alt="HighPop" style="height:28px;vertical-align:middle;margin-right:8px"/>Dashboard</div>
  <div class="hdr-right">
...
<div id="authWrap" class="auth-wrap">
  <div class="auth-card">
    <h2><img src="/favicon.ico" alt="HighPop" style="height:32px;vertical-align:middle;margin-right:8px"/>Dashboard</h2>
...
function connect(){
  ...
  if(TOKEN){localStorage.setItem('highpop_token',TOKEN);loadAll();}
}
(function(){
  const t=localStorage.getItem('highpop_token')||localStorage.getItem('wgs_token');
...
let sortMode=localStorage.getItem('highpopSortMode')||localStorage.getItem('wgsSortMode')||'name-asc';
function setSortMode(v){sortMode=v;localStorage.setItem('highpopSortMode',v);if(lastServerList)renderServers(lastServerList);}
...
async function fetchPerf(id){
  try{
    ...
    sp.innerHTML=`<polyline points="${pts}" fill="none" stroke="#f05a28" stroke-width="1.5" vector-effect="non-scaling-stroke"/>`;
  }catch(e){}
}
