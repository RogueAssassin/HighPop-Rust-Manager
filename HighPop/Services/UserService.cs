...
public class UserService
{
    ...
    private void EnsureSchema()
    {
        ...
        if (n == 0)
            CreateUser("admin", Convert.ToHexString(RandomNumberGenerator.GetBytes(32)), UserRole.Admin);
        MigratePlaintextTokens(db);
    }

    private static void MigratePlaintextTokens(SqliteConnection db)
    {
        var legacy = new List<(int Id, string Token)>();
        using (var select = db.CreateCommand())
        {
            select.CommandText = "SELECT id, token FROM users WHERE token NOT LIKE 'dpapi:%'";
            using var reader = select.ExecuteReader();
            while (reader.Read()) legacy.Add((reader.GetInt32(0), reader.GetString(1)));
        }
        foreach (var row in legacy)
        {
            using var update = db.CreateCommand();
            update.CommandText = "UPDATE users SET token=$t WHERE id=$id";
            update.Parameters.AddWithValue("$t", ProtectToken(row.Token));
        ...
        try
        {
            ...
            while (r.Read())
                list.Add(new WgsUser
                {
                    ...
                    Token     = UnprotectToken(r.GetString(3)),
    ...
    public bool ValidateToken(string token, out WgsUser? user)
    {
        ...
        try
        {
            using var db  = OpenDb();
            using var cmd = db.CreateCommand();
            cmd.CommandText = "SELECT id, username, role, token, is_enabled, last_login FROM users WHERE is_enabled=1";
            using var r = cmd.ExecuteReader();
            while (r.Read())
            {
                var candidate = UnprotectToken(r.GetString(3));
                if (!FixedTimeEquals(token, candidate)) continue;
                user = new WgsUser
                {
                    Id        = r.GetInt32(0),
                    Username  = r.GetString(1),
    ...
    public void CreateUser(string username, string password, UserRole role)
    {
        ...
        try
        {
            ...
            cmd.Parameters.AddWithValue("$t", ProtectToken(NewToken()));
            cmd.ExecuteNonQuery();
    ...
    private static string ProtectToken(string token)
        => string.IsNullOrEmpty(token) ? string.Empty : "dpapi:" + EncryptionService.Encrypt(token);

    private static string UnprotectToken(string stored)
        => stored.StartsWith("dpapi:", StringComparison.Ordinal)
            ? EncryptionService.Decrypt(stored[6..])
            : stored; // legacy plaintext migration support
