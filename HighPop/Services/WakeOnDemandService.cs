...
public sealed class WakeOnDemandService : IDisposable
{
    ...
    {
        if (!server.WakeOnDemand || !server.WakeOnDemandPortTrigger
            || server.ServerPort is <= 0 or > 65535) return;
    ...
    private async Task ListenAsync(GameServer server, CancellationToken ct, TimeSpan delay = default)
    {
        ...
        Task<bool>? udpTask = null;
        Task<bool>? tcpTask = null;
        try
        {
            udpTask = WaitForUdpAsync(server.ServerPort, linked);
            tcpTask = WaitForTcpAsync(server.ServerPort, linked);
            var winner = await Task.WhenAny(udpTask, tcpTask);
            var signaled = await winner;
            if (!signaled)
            {
                var other = ReferenceEquals(winner, udpTask) ? tcpTask : udpTask;
                signaled = await other;
            }
            triggered = signaled && !ct.IsCancellationRequested;
        }
        catch (OperationCanceledException) { }
        finally
        {
            innerCts.Cancel(); // shut down whichever listener didn't fire
            if (udpTask != null && tcpTask != null)
            {
                try { await Task.WhenAll(udpTask, tcpTask); } catch { }
            }
        }
    ...
    private static async Task<bool> WaitForUdpAsync(int port, CancellationToken ct)
    {
        try
        {
    ...
    private static async Task<bool> WaitForTcpAsync(int port, CancellationToken ct)
    {
