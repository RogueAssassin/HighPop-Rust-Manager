...
public class ModManagerService
{
    ...
    /// <summary>Download the latest stable Windows Carbon release and extract it over RustDedicated.exe.</summary>
    public async Task InstallCarbonAsync(string installPath,
                                         IProgress<(int pct, string msg)>? progress = null)
    {
        if (!File.Exists(Path.Combine(installPath, "RustDedicated.exe")))
            throw new InvalidOperationException("Install the Rust dedicated server before installing Carbon.");
        ...
        var asset = release.RootElement.GetProperty("assets").EnumerateArray()
            ...
            .FirstOrDefault(a => a.Name.EndsWith("Carbon.Windows.Release.zip", StringComparison.OrdinalIgnoreCase)
        ...
        var tempZip = Path.Combine(Path.GetTempPath(), $"carbon_{Guid.NewGuid():N}.zip");
        try
        {
            await File.WriteAllBytesAsync(tempZip, bytes);
            Report(progress, 65, "Extracting Carbon into the Rust server...");
            ZipFile.ExtractToDirectory(tempZip, installPath, overwriteFiles: true);
        }
        finally
        {
    ...
    {
        var dir = Path.Combine(installPath, "carbon", "plugins");
                                        ...
                                        IProgress<(int pct, string msg)>? progress = null)
    {
        ...
        if (!File.Exists(eulaPath))
            await File.WriteAllTextAsync(eulaPath, "#Generated by HighPop\neula=true\n");
    ...
    private static void EnsureEula(string installPath)
    {
        var eulaPath = Path.Combine(installPath, "eula.txt");
        if (!File.Exists(eulaPath))
            File.WriteAllText(eulaPath, "#Generated by HighPop — you accept the Minecraft EULA\neula=true\n");
    }
