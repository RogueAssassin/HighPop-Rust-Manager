...
public class TemplateService
{
    ...
    /// <summary>Export a single template to .json or .hpt (ZIP with image).</summary>
    ...
    /// <summary>
    /// Import templates from a .json, .hpt, or legacy .wgst file.
    ...
    public (int Count, string? ImagePath) Import(string filePath, string imageOutputDir)
    {
        ...
        if (extension.Equals(".hpt", StringComparison.OrdinalIgnoreCase)
            || extension.Equals(".wgst", StringComparison.OrdinalIgnoreCase))
        {
            using var zip = ZipFile.OpenRead(filePath);
            var jsonEntry = zip.GetEntry("template.json")
                ?? throw new InvalidOperationException("Invalid HighPop template: missing template.json");
