...
public static class PluginCompilerService
{
    public static (IGamePlugin? plugin, string error) CompileAndLoad(string csFilePath)
    {
        ...
        var tmpDll = Path.Combine(Path.GetTempPath(),
            $"highpop_plugin_{Path.GetFileNameWithoutExtension(csFilePath)}_{Guid.NewGuid():N}.dll");
        ...
        // Note: temp file stays on disk until HighPop exits (file is locked by the loaded assembly)
    ...
    private static List<MetadataReference> BuildReferences()
    {
        ...
        // 3. HighPop own assemblies — in single-file publish the DLLs are extracted to a temp
