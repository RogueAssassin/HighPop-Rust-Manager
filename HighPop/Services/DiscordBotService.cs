...
///   3. Copy the Bot Token and the Channel ID into HighPop Settings → Discord Bot
///
...
public class DiscordBotService : IDisposable
{
    ...
    /// Persisted to disk so a HighPop restart edits the same message instead of leaving it behind and posting a new one.</summary>
    ...
    private static object BuildStatusEmbed(List<Models.GameServer> servers)
    {
        ...
        return new
        {
            ...
            color       = 0x1F6FEB,
            fields,
            footer      = new { text = "HighPop Rust Manager · updates every minute" },
    ...
    private string BuildHelp() =>
        $"**HighPop Rust Manager — Remote Commands**\n" +
    ...
    public async Task<bool> TestConnectionAsync()
    {
        ...
        try
        {
            _http.DefaultRequestHeaders.Authorization =
                new AuthenticationHeaderValue("Bot", BotToken);
            await SendMessage("🧪 HighPop Discord bot connection test — OK!");
            return true;
        }
        catch { return false; }
    }
