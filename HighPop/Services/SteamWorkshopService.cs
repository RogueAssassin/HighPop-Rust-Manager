...
public class SteamWorkshopService
{
    ...
    public Task<List<WorkshopItem>> GetInstalledItemsAsync(GameServer _, IGamePlugin plugin)
    {
        ...
        foreach (var dir in Directory.GetDirectories(workshopDir))
        {
            ...
            var meta = Path.Combine(dir, "highpop_workshop_meta.json");
            if (!File.Exists(meta)) meta = Path.Combine(dir, "wgs_workshop_meta.json");
            if (File.Exists(meta))
            {
                try
                {
        ...
        IProgress<(int pct, string msg)>? progress = null)
    {
        ...
        {
            try { File.WriteAllText(Path.Combine(itemPath, "highpop_workshop_meta.json"),
    ...
    /// <summary>
    /// Returns the mods whose Steam time_updated is newer than what HighPop last downloaded.
