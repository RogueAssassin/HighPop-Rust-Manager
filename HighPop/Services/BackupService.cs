...
public class BackupService
{
    ...
    public async Task<BackupEntry> CreateBackupAsync(GameServer server)
    {
        ...
        var serverRoot = Path.GetFullPath(server.InstallPath)
            .TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar);
        var serverRootWithSeparator = serverRoot + Path.DirectorySeparatorChar;
        var current  = new Dictionary<string, FileState>();
        foreach (var dir in saveDirs)
        {
            if (!Directory.Exists(dir)) continue;
            var fullDir = Path.GetFullPath(dir);
            if ((!fullDir.StartsWith(serverRootWithSeparator, StringComparison.OrdinalIgnoreCase)
                 && !fullDir.Equals(serverRoot, StringComparison.OrdinalIgnoreCase))
                || ContainsReparsePointBelowRoot(serverRoot, fullDir))
                throw new InvalidDataException("Backup paths must stay inside the server installation folder and cannot use junctions.");
    ...
    public async Task RestoreBackupAsync(GameServer server, string zipPath)
    {
        ...
        await Task.Run(() =>
        {
            var restoreRoot = Path.GetFullPath(server.InstallPath)
                .TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar)
            ...
            foreach (var entry in chain)
            {
                ...
                foreach (var zipEntry in zip.Entries)
                {
                    ...
                    var dest = Path.GetFullPath(Path.Combine(server.InstallPath, zipEntry.FullName));
                    if (!dest.StartsWith(restoreRoot, StringComparison.OrdinalIgnoreCase))
                        throw new InvalidDataException($"Backup contains an unsafe path: {zipEntry.FullName}");
                    var trustedRoot = restoreRoot.TrimEnd(Path.DirectorySeparatorChar);
                    if (ContainsReparsePointBelowRoot(trustedRoot, dest))
                        throw new InvalidDataException($"Backup path crosses a junction or symbolic link: {zipEntry.FullName}");
                ...
                foreach (var rel in deletedList)
                {
                    try
                    {
                        var p = Path.GetFullPath(Path.Combine(server.InstallPath, rel));
                        var trustedRoot = restoreRoot.TrimEnd(Path.DirectorySeparatorChar);
                        if (p.StartsWith(restoreRoot, StringComparison.OrdinalIgnoreCase)
                            && !ContainsReparsePointBelowRoot(trustedRoot, p)
                            && File.Exists(p))
                            File.Delete(p);
                    }
                    catch { }
    ...
    private static string[] GetSaveDirectories(GameServer server)
    {
        ...
        if (!string.IsNullOrWhiteSpace(server.BackupSavePath))
        {
            ...
            foreach (var relative in server.BackupSavePath.Split(
                ';', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries))
            {
                if (Path.IsPathRooted(relative))
                    throw new InvalidDataException("Backup paths must be relative to the server installation folder.");
                var full = Path.GetFullPath(Path.Combine(root, relative));
                if (!full.StartsWith(rootWithSeparator, StringComparison.OrdinalIgnoreCase)
                    && !full.Equals(root, StringComparison.OrdinalIgnoreCase))
                    throw new InvalidDataException("Backup path must stay inside the server installation folder.");
                if (ContainsReparsePointBelowRoot(root, full))
                    throw new InvalidDataException("Backup path cannot pass through a junction or symbolic link.");
                result.Add(full);
            }
            return result.ToArray();
        }

        return [Path.Combine(server.InstallPath, "server")];
    }
    ...
    private static bool ContainsReparsePointBelowRoot(string root, string candidate)
    {
        try
        {
            ...
            while (!string.IsNullOrEmpty(cursor)
                   && !cursor.Equals(root, StringComparison.OrdinalIgnoreCase))
            {
