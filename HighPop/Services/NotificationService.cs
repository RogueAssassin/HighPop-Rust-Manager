...
public class NotificationService
{
    ...
    public void Save()
    {
        ...
        System.IO.File.WriteAllText(temp, JsonConvert.SerializeObject(data, Formatting.Indented));
        System.IO.File.Move(temp, _settingsFile, overwrite: true);
    }
    ...
    private async Task SendEmailAsync(string subject, string body)
    {
        try
        {
            ...
            var html = $"""
                ...
                <h2 style="color:#f05a28">{System.Net.WebUtility.HtmlEncode(subject)}</h2>
                <p>{System.Net.WebUtility.HtmlEncode(body)}</p>
                <hr style="border-color:#30363d"/><p style="color:#8b949e;font-size:12px">HighPop Rust Manager</p>
                </body></html>
                """;
    ...
    public async Task<bool> TestEmailAsync()
    {
        ...
        try
        {
            await SendEmailAsync("🧪 HighPop Test Email", "HighPop Rust Manager email notifications are working!");
            return true;
        }
        catch { return false; }
    }
            ...
            var payload = new
            {
                embeds = new[]
                {
                    new
                    {
                        ...
                        footer = new { text = "HighPop Rust Manager" }
                    }
                }
            };
