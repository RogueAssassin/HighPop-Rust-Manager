...
public class RemoteMachineService : IDisposable
{
    ...
    private void Load()
    {
        ...
        try
        {
            var array = JArray.Parse(File.ReadAllText(_machinesFile));
            var needsMigration = false;
            foreach (var item in array.OfType<JObject>())
            {
                var stored = item[nameof(MachineDefinition.Token)]?.Value<string>() ?? string.Empty;
                if (!string.IsNullOrEmpty(stored)
                    && !stored.StartsWith("dpapi:", StringComparison.Ordinal))
                    needsMigration = true;
                item[nameof(MachineDefinition.Token)] = stored.StartsWith("dpapi:", StringComparison.Ordinal)
                    ? EncryptionService.Decrypt(stored[6..])
                    : stored;
            }
            var list = array.ToObject<List<MachineDefinition>>();
            if (list == null) return;
            lock (_machinesLock)
    ...
    private void Save()
    {
        ...
        foreach (var item in array.OfType<JObject>())
        {
            var token = item[nameof(MachineDefinition.Token)]?.Value<string>() ?? string.Empty;
            item[nameof(MachineDefinition.Token)] = string.IsNullOrEmpty(token)
                ? string.Empty
                : "dpapi:" + EncryptionService.Encrypt(token);
        }
        var temp = _machinesFile + ".tmp";
        File.WriteAllText(temp, array.ToString(Formatting.Indented));
        File.Move(temp, _machinesFile, overwrite: true);
    }
