...
public class RedMPlugin : GamePluginBase
{
    ...
    // that up itself on first run (setup wizard, recipe choice, resource downloads). HighPop's job
    ...
    // serverProfile lets each HighPop-managed server load its own txAdmin profile instead of every
